...
DispatchExecutor + CLIBackend → real ``codex exec`` subprocess) against
``examples/jokes/workflow.yaml`` with the YAML edited to use a
``transport: cli`` preset. Skipped at collection time when ``codex``
...
test (still skipping when ``codex`` is missing); the marker exists so
...
def test_jokes_roundtrip_cli_transport(tmp_path):
    ...
    Failure modes worth catching: cli regressed (`codex exec` flag
    change), model name retired, gate validator drift, prompt template
    breakage end-to-end.
    """
    yaml_path = _stage_jokes(
        tmp_path, transport="cli", model="gpt-5.6-luna",
    )
