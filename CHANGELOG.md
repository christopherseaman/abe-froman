...
- Collapsed the gated-node eval/decision node pair (and the separate combined eval+decide factory for dynamic parents) into ONE `Command`-returning gate node, and replaced the fan-out conditional-edge router with a `_fan_<id>` dispatcher node. The gate is still registered under the existing `_eval_<id>` name (JSONL/event node ids unchanged; `_decide_<id>` nodes are gone). Internal-only for any workflow — no schema or CLI surface change — but three observable effects:
  ...
  - `sqrlly graph` now renders the AUTHORED topology via `view`'s schema reconstruction instead of the compiled LangGraph (whose gated/fan-out routing is runtime `Command(goto=...)` and would render edge-less). Net improvement: `graph` now DRAWS declared `route:` edges (a long-standing gap) and fan-out hexagons, gated nodes are stroke-styled, and compiled synthetics (`_eval_`/`_decide_`/`_sub_`) no longer appear. `graph` and `view` now show the same topology; the Mermaid header changes from `graph TD` to `flowchart TB`.
...
- `settings:` now rejects unknown keys (`extra=forbid`, consistent with every other model) — a typo'd or stale setting fails loud at load instead of being silently ignored.
