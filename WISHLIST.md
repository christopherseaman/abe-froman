...
- [ ] **(30) `_make_final_fan_out_node` is a polling barrier that fights LangGraph's scheduler** (`compile/dynamic.py:322-349`). The first final-node wrapper returns `{}` until all Send-branch children land in `completed_nodes`/`failed_nodes`. Each completing branch re-fires the body, which defers again — silent extra invocations + a `node_id in completed` guard at line 346 to prevent double-execution. The intended semantics ("wait until all branches settle") is exactly what a dedicated aggregator with multiple incoming edges expresses natively. Fix cost: medium (topology rewrite during `build_workflow_graph`). Benefit: removes two guards, eliminates extra invocations, aligns with LangGraph's fan-in idiom.

- [ ] **(31) `--resume` discards the checkpointer instead of trusting it** (`cli/main.py:269-291`). Reads `channel_values` from prior checkpoint, builds a cleaned state dict, calls `cp.adelete_thread(thread_id)`, then re-streams from initial-state-like dict. Effectively replays the whole graph (why `completed_nodes` accumulates duplicates per WISHLIST 26). The LangGraph-native pattern for `AsyncSqliteSaver` is to pass the existing `thread_id` to `astream` without modifying state — checkpointer resumes from the last super-step boundary. Pattern was inherited from the pre-`AsyncSqliteSaver` era (per WISHLIST 19) and never updated. Directly resolves the DAG case of WISHLIST 26 without new infrastructure. Goto-driven case still needs the design call documented there.

- [ ] **(32) `completed_nodes` / `failed_nodes` use `operator.add` reducer** (`runtime/state.py:43-53`) → unbounded growth + O(n) `in` checks scattered through `nodes.py` and `dynamic.py`. A set-union reducer (or `dict[str, bool]`) makes membership O(1) and structurally prevents duplicates. Currently chosen for JSONL event derivation in `logging.py` (delta-new entries = "completed this super-step"), but a set-union reducer preserves that semantic. Touches every guard; needs careful migration. Pairs with #31 as the structural cause of `--resume` accumulation.

- [ ] **(33) `all_deps_completed` is a manual polling barrier for join nodes** (`compile/nodes.py:66-75`). Multi-predecessor nodes whose preds are gated get triggered N times (once per pred Decision firing `Command(goto=successor)`); the guard defers N-1 of those. The function's own comment says "subsequent pred completions re-fire this node." Native LangGraph `add_barrier_node` / multi-edge join expresses this topologically. Fix cost: high (graph-level join detection during `build_workflow_graph`). Defer until the manual barrier surfaces real pain — current behavior is correct, just non-idiomatic.
