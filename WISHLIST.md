...
- [ ] **Remove `StubBackend` from production code** — surfaced 2026-05-01 during README pass. The class returns plausible-looking fake output (`[prompt-stub] model=... prompt_length=...`) and is the auto-detect last-resort fallback; it's the same antipattern `feedback_no_fake_backends.md` forbids in tests, just shipped in production. **Audit:** 7 test files use it (1 directly tests the class, 4 use it as a stand-in for "any backend that returns something" in orchestration tests, 2 use it for factory-shape assertions). **Migration plan:** (a) delete `runtime/executor/backends/stub.py` + factory case + `auto_detect_executor` fallback (replace fallback with a loud `RuntimeError` naming remediation); (b) migrate stand-in tests to `_ECHO` script execution where prompt content doesn't matter; delete the `prompt_length` substitution test since `tests/unit/runtime/test_prompt.py::TestPromptExecutor` already covers Jinja rendering against real subprocess paths; (c) breaking change in CHANGELOG — `--executor stub` and `executor: stub` no longer accepted. ~150 LOC delta across 9 files. Mechanical; coupled with the policy conflict below.
