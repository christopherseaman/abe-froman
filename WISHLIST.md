...
- [ ] **(13) `AnthropicBackend` doesn't surface `tokens_used`** (`runtime/executor/backends/anthropic.py:149`) — `ExecutionResult(output=...)` drops the SDK's usage data. The `anthropic` SDK exposes `resp.usage.input_tokens` / `output_tokens`; threading them onto `ExecutionResult` is ~3 LOC + a schema field. Pairs with the unified `ExecutionResult` refactor (Refactoring section) and Samus's "direct Claude API token tracking" requirement.
