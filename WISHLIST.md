...
- [ ] **(20) DeepSeek live tests (`tests/unit/runtime/test_openai_backend.py::TestDeepSeekLive`) flake** (observed 2026-05-06) — `test_send_prompt_returns_text` and `test_close_is_idempotent` time out at 30s when run against the live API. Pre-existing on this branch (confirmed via `git stash` baseline check); appears unrelated to the 6-commit branch's work. Likely either the `deepseek-v4-flash` model name no longer resolves, the upstream is rate-limiting, or this tier has reduced availability. Investigate: pin a known-good model (e.g. `deepseek-chat` from the docs) instead of `deepseek-v4-flash`, OR gate the live tests behind an opt-in marker so they don't pollute green-on-CI. Two failures total; the offline `TestErrorMapping` and `TestKeyResolution` classes are unaffected.
