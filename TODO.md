...
- [ ] **`sqrlly init --example <name>` — scaffold a bundled example** — DESIGNED (spec: `docs/superpowers/specs/2026-06-23-init-example-scaffold-design.md`), not yet built. `pipx`/`uv tool` users have no repo checkout, so they can't run any bundled example (examples don't ship in the wheel today). Plan: `force-include` a curated run-essential set (`jokes`, `route_classify`, `explicit_join`, `pipeline_style`) into the wheel at `sqrlly/_examples/…` (mirroring `SKILLS.md → _skill.md`), add `init --example <name> [dir]` + `init --list-examples`, and rewrite `examples/<name>/` URL prefixes at scaffold time so the copy is flat + self-contained. Heavy examples (`absurd-paper`) and a separate `sqrlly-examples` package / `[examples]` extra are out of scope (extras gate packages, not wheel data files; revisit only for heavy content).
