...
def prepare_resume_state(
    ...
    # Remove any phases that failed from completed (shouldn't overlap, but be safe)
    completed = [p for p in completed if p not in failed]

    # Only keep config phase IDs that still exist
    valid_ids = {p.id for p in config.phases}
    completed = [p for p in completed if p in valid_ids or "::" in p]

    # Preserve outputs/scores only for completed phases
    phase_outputs = {
...
def prepare_start_state(
    ...
    # Verify all upstream phases were completed in the saved state
