...
def _yaml() -> YAML:
    ...
    Deliberately duplicated in ``scripts/migrate_legacy_executor_to_
    presets.py``: that script is a PEP-723 standalone (``uv run``-able
    with no install) and cannot import from the ``sqrlly`` package.
    """
    y = YAML(typ="rt")
...
def _walk_and_migrate(root: CommentedMap, changes: list[str]) -> None:
    ...
    # NOTE: the legacy executor:/default_model:/Node.model: →
    # settings.presets migration is a one-shot transform and lives in
    # scripts/migrate_legacy_executor_to_presets.py (run separately),
    # not in this longer-lived base migrator.
